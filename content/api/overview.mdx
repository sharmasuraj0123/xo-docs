---
title: XO Cowork API Architecture and Conventions
icon: Info
...
`xo-cowork-api` is a Python 3.12 FastAPI app that runs inside every Coworker space on port `5002`. The frontend (Tauri desktop, browser, mobile, or any AI client) talks to it over plain HTTP plus Server-Sent Events.
...
These headers belong to the remote proxy boundary and are ignored by Cowork API
when no validating proxy is present. They are **not** the same as the
`xo-swarm-api` Bearer token that cowork-api itself holds for upstream calls. That
