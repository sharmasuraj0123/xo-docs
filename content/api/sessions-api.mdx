...
~/claude-space/                           ← Claude Code adapter
└── <agent_id>/
    └── sessions/
...
~/claude-space/<agent_id>/.sessions/<session_id>.json
```
...
The exact per-message shape varies by runtime. The conversion runs through `services.space_agent.messages.convert_messages()`, which normalizes Claude Code stream-json plus OpenClaw chat-completions JSONL into the same envelope.
...
find_session_file("abc-123"):
        ...
        ├── ~/claude-space/*/sessions/abc-123.jsonl        found? → return
...
Update the working directory associated with a session (used by the project / space switcher in the UI).
...
2. If not found, fall through to update_claude_session_directory
   (same shape but in ~/claude-space/* tree, with legacy `.sessions/{id}.json` fallback).
