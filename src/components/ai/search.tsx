    ...
    <>
      {!isLoading && messages.at(-1)?.role === "assistant" && (
        <button
          ...
          onClick={() => regenerate()}
        >
          <span className="icon-[ph--arrow-clockwise] size-4" />
          Retry
        </button>
      )}
      <button
    ...
    >
      ...
      {isLoading ? (
        <button
          ...
          onClick={stop}
        >
          <span className="icon-[ph--spinner] size-4 animate-spin text-fd-muted-foreground" />
          Abort Answer
        </button>
      ) : (
        <button
        ...
        >
          <span className="icon-[ph--paper-plane-tilt] size-4" />
        </button>
      )}
    </form>
  );
    ...
    <div {...props}>
      ...
      {searchCalls.map((call) => {
        return (
          ...
          >
            <span className="icon-[ph--magnifying-glass] size-4" />
