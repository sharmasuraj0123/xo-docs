...
Open **Inbox → Sharing**, or use `/space/#/inbox/sharing`. The older `#/sharing` and `#/projects/sharing` routes also resolve here. Sharing activity has its own neighboring **Sharing activity** page.
...
For a project already on this machine, you can also open [Projects → Manage](/docs/space/space-walk/manage), choose **Share** on its card, enter the recipient's **Space ID**, and submit. The inline form shows the result without leaving Manage.

From the Sharing page:
...
| **needs GitHub** | **Connect GitHub** opens **Setup → Connectors**. Configure the required GitHub access, then let the next check retry. |
...
The detail panel shows the repository, watched branch, latest check, up to five recent commits, and members. New commits are marked **new**. **Open in List** switches to that project's expanded drawer in **Projects → Data → List**.
