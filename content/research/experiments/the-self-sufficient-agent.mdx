...
Here is what a bare agent (empty space, no README, no contract, no memory) solves on each kind of task. A do-nothing answer scores 0% on every one of them, so anything above the floor is real work.
