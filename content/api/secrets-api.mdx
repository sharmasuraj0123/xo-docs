...
<Callout type="warn">
`GET /api/secrets/env` returns plaintext values, including secrets. Only the space itself should expose this surface; never tunnel it past the trust boundary without an auth layer.
</Callout>
...
If you want to update a single key without losing comments or unrelated keys, use the onboarding "Save Key" flow inside `routers/space_agent/config.py`, which goes through `upsert_env_entry()` (line-level edit). That path is not part of `/api/secrets/*`.
...
- The space daemon reads and writes it directly. There is no DB, no encryption-at-rest, no per-user scoping.
- Space = trust boundary. Anyone who can reach the API endpoint can read every secret.
