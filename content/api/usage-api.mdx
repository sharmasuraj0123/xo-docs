...
These endpoints expose the OpenClaw-only usage parser used by `services/space_agent/adapters/openclaw/usage.py`. Same data feeds `/api/usage`, but unfiltered and without Claude Code merge.
...
```
1. Walk OpenClaw JSONL session files under the space.
