...
Open **Projects → Data → Tree**, or use `/space/#/projects/data/tree`. The older `#/tree` link also opens this view.
...
Click a file to open the shared [file preview](/docs/space/space-walk/files-explorer#preview-a-file). The Tree's camera and expansion state stay in place. Close the preview to continue browsing, or use the **List | Graph | Tree** switch within Data to change views.

## Controls

| Control | Effect |
|---|---|
| Active **Filter tree by name…** query | Matches names case-insensitively and reveals matching paths. A suffix such as `.md` works as ordinary name text. Clear the active query to return to the unfiltered tree. |
