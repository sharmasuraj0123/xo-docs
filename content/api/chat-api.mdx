...
```jsonc
{
  ...
  // OPTIONAL: space path hint (alternative to agent_id)
  ___JSON_WORKSPACE_KEY___: "/Users/me/xo-projects/blackhole",              // absolute path
...
If `agent_id` is missing, it's a new session, and `space` is provided:

```
ws_path = Path(space).expanduser().resolve()
if   ws_path startswith ~/xo-projects/{X}/...   →  agent_id = X
elif ws_path startswith ~/claude-space/{X}/...  →  agent_id = X
else                                             →  agent_id = None
```
...
Use `GET /api/messages/{session_id}`. See the [Sessions API](/api-reference/sessions-api).
