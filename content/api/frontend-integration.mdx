...
This page is a copy-pasteable cookbook. Every snippet is TypeScript, uses standard `fetch` plus `EventSource`, and assumes you have already resolved the `BASE` URL (see [Overview](/api-reference/overview)).
...
```typescript
type SpaceConfig = { roots: Record<string, string>; default: string };

async function createProject(slug: string, displayName: string, description = "") {
  const cfg = await api<SpaceConfig>("/api/config/workspace");
...
```typescript
async function uploadFile(file: File, space?: string) {
  const fd = new FormData();
  fd.append("file", file);
  if (space) fd.append(___JSON_WORKSPACE_KEY___, space);
...
See [Auth API](/api-reference/auth-api), [Config API](/api-reference/config-api), and [Connectors API](/api-reference/connectors-api) for the flows that follow.
...
<Callout type="warn">
**Always send absolute paths.** Every filesystem endpoint resolves with `Path.resolve()` against the space process CWD. Sending relative paths gives unpredictable results.
</Callout>
