...
<Accordions>
  <Accordion title="Encryption pipeline">
    Backups pipe `tar` output directly into `gpg --symmetric --cipher-algo AES256` — no unencrypted intermediate files touch disk. The encrypted archive is a single `.tgz.gpg` file before chunking.
  </Accordion>
  <Accordion title="Chunking for GitHub">
    GitHub enforces a 100 MB per-file limit. The skill splits encrypted archives into 95 MB chunks (leaving overhead margin) named `part-000.gpg`, `part-001.gpg`, etc. During restore, chunks are reassembled in order before decryption.
  </Accordion>
  <Accordion title="Manifest format">
    Each backup version lives in its own timestamped folder inside the transport repo. A `manifest.json` file tracks the timestamp, chunk count, total size, SHA-256 checksum, and an ordered parts list for integrity verification during restore.
  </Accordion>
