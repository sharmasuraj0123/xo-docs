...
1. **Clerk poll-token flow** (`/xo-auth/*`): how the Space desktop signs the user in to `xo-swarm-api`.
...
<Callout type="warn">
Tokens are stored in-memory only (`auth_state` in `routers/auth.py`). There is no refresh loop. Restart the space process and you'll need to re-consume.
</Callout>

## 1. Clerk poll-token flow

Triggered when no `XO_API_KEY` env var is set. The Tauri shell opens the browser, the user signs in to Clerk on swarm, and the space daemon consumes the resulting one-time `poll_token` to mint a Bearer token for `xo-swarm-api`.
...
<Steps>
  ...
  <Step>
    ...
    ```
    GET /xo-auth/whoami
    headers: (none from the client; space injects Bearer from auth_state)
...
If `XO_AUTH_SESSION_ID` and `XO_POLL_TOKEN` env vars are both set, the FastAPI lifespan hook runs `consume_auth_flow()` automatically before the first request lands. Useful for headless boot in CI or Coder spaces.
...
| `GET` | `/health` | Detailed process diagnostics, including stage, CLI paths, sanitized auth state, and active-session count. |
...
~/.space/state.json     onboarding flags
~/.space/github_token.json gh CLI cached token
