...
The files API operates directly on the space filesystem, clamped to `$HOME`. Use it for editor flows, attachments, and project creation.
...
Multipart file upload. Stores the file in either the explicit `space` directory or `~/uploads/` as a fallback.
...
| `space` | `str` | no | Absolute destination directory. If empty, defaults to `~/uploads/`. Created if missing. |
...
- **`space` not clamped.** Unique to this endpoint; everything else clamps to `$HOME`.
...
This is the project-creation flow. It runs `services.space_agent.project_layout.scaffold_project()`:
...
|---|---|---|---|---|---|
| `/api/files/upload` | no (space optional) | `multipart/form-data` | partial (`space` not clamped) | yes (sha256 dedup) | writes to filesystem |
