...
icon: Zap
description: "Verify a Cowork API instance, discover its workspace configuration, and stream a first chat turn."
---
...
overview](/more/xo-cowork-api/overview#auth-model).
...
<Steps>
  <Step>
    ...
    If this request cannot connect, confirm that the Coworker workspace is running
...
API](/more/xo-cowork-api/auth-api) for Clerk and Claude/Codex bootstrap flows, and
`GET /api/models` from the [Config API](/more/xo-cowork-api/config-api) to populate
...
- Read the [Overview](/more/xo-cowork-api/overview) for remote auth, content types,
  and the shared error envelope.
- Use the [Chat API](/more/xo-cowork-api/chat-api) for reconnect, heartbeat, abort,
  and full event semantics.
- Copy the production-oriented TypeScript patterns in [Frontend
  Integration](/more/xo-cowork-api/frontend-integration).
- Use the [Files API](/more/xo-cowork-api/files-api) and [Sessions
  API](/more/xo-cowork-api/sessions-api) to add project navigation and history.
