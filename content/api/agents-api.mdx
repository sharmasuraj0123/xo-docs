...
The agents API is a CRUD wrapper over a single JSON file: `~/.openclaw/openclaw.json`. Each agent is an entry under `agents[]` with a unique `id`. The file is the source of truth for which OpenClaw agents the space knows about.
...
Every write is a **full read-merge-write** of the entire file. There is no record-level locking. The space's single-user assumption keeps this fine in practice.
...
<Callout type="warn">
Two concurrent `PATCH` calls can race because the implementation does a full read-merge-write of `openclaw.json` per call. The space assumption is one writer (the single user driving the UI), so this is fine in practice. If you build a multi-writer client, serialize calls client-side.
</Callout>

## Related: agent selection in chat

The agents listed here are OpenClaw agents (the `services.space_agent.openclaw_store` registry). They are what `agent_id` and `agent_name` in `POST /api/chat/prompt` reference when running OpenClaw turns.
...
These are read-only at the `/api/skills` and `/api/tools` endpoints (in `routers/space_agent/misc.py`). To list or modify skills you would edit the manifest files on disk; there is no PATCH endpoint for them today.
